...
set num_pkts [expr [lindex $argv 1]];
...
if { [string compare [lindex $argv 3] "Cubic"] == 0 } {

	set sourceAlg Cubic;
} elseif { [string compare [lindex $argv 3] "Vegas"] == 0 } {
...
set interval [expr [lindex $argv 4]];
# target in ms.
#set target [delay_parse 5ms]
set target [expr [lindex $argv 5]];

Queue/CoDel set init_interval_ 1;
Queue/CoDel set detect_interval_ [expr $RTT/10]
...
if {$recordFlowInfo != 0} {
	...
	set flow_startingtime [open flow-startingtime_codel.txt w]
	set recordall [open traceall-codel.ta w]
	$ns trace-all $recordall
}

proc finish {} {
		global ns enableNAM flow_startingtime namfile recordFlowInfo
		$ns flush-trace
		if {$enableNAM != 0} {
			close $namfile
			exec nam out.nam &
