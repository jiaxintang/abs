...
for {set k 0} {$k < [expr $ROUND]} {incr k} {

	set type [expr int(fmod($k,3))];

	if {$type == 0} {
		set step 30;
		
		for {set i $loc} {$i < [expr $loc+$step]} {incr i} {
	
#			$ns at [expr $k*1.0/$ROUND*$simulationTime] "$ftp($i) produce 500"
			$ns at [expr $k*1.0/$ROUND*$simulationTime+($i-$loc)/$step] "$ftp($i) start"
			$ns at [expr ($k+1)*1.0/$ROUND*$simulationTime] "$ftp($i) stop"
#			puts "$i with $loc, under $type/$k"
		}
		set loc [expr $loc+$step];
	}
...
#			$ns at [expr $k*1.0/$ROUND*$simulationTime] "$ftp($i) produce 50"
			$ns at [expr $k*1.0/$ROUND*$simulationTime+($i-$loc)/$step] "$ftp($i) start"
			$ns at [expr ($k+1)*1.0/$ROUND*$simulationTime] "$ftp($i) stop"
#			puts "$i with $loc, under $type/$k"
		}
...
#			$ns at [expr $k*1.0/$ROUND*$simulationTime] "$ftp($i) produce 1000"
			$ns at [expr $k*1.0/$ROUND*$simulationTime+($i-$loc)/$step] "$ftp($i) start"
			$ns at [expr ($k+1)*1.0/$ROUND*$simulationTime] "$ftp($i) stop"
#			puts "$i with $loc, under $type/$k"
		}
...
#	puts "produce time: [expr $k*1.0/$ROUND*$simulationTime]";
}
