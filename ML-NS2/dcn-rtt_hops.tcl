...
for {set i 0} {$i < $N} {incr i} {
	$ns duplex-link $n($i) $nqueue $inputLineRate [expr $RTT/2] DropTail
	$ns duplex-link-op $n($i) $nqueue queuePos 0.25
}

for {set i 0} {$i < [expr $HOPS-1]} {incr i} {

	$ns duplex-link $h($i) $h([expr $i+1]) $lineRate [expr $HOP_RTT/2] DropTail;
	$ns duplex-link-op $h($i) $h([expr $i+1]) queuePos 0.25;
#	$ns duplex-link $h($i) $nclient $lineRate [expr $RTT/8] DropTail;
#	$ns duplex-link-op $h($i) $nclient queuePos 0.25;
	$ns queue-limit $h($i) $h([expr $i+1]) 2;
#	$ns queue-limit $h($i) $nclient 2;
}

for {set i 2} {$i < $HOPS} {incr i} {

	$ns duplex-link $h(0) $h($i) $lineRate [expr $HOP_RTT/2] DropTail;
	$ns duplex-link-op $h(0) $h($i) queuePos 0.25;
	$ns queue-limit $h(0) $h($i) 2;
}

$ns duplex-link $h(0) $nclient $lineRate [expr $HOP_RTT/2] DropTail;
$ns duplex-link-op $h(0) $nclient queuePos 0.25;
$ns queue-limit $h(0) $nclient 2;

# first and last nodes
$ns simplex-link $nqueue $h(0) $bottleneckRate [expr $RTT/2] $switchAlg;
$ns simplex-link $h(0) $nqueue $bottleneckRate [expr $RTT/2] DropTail;
$ns queue-limit $nqueue $h(0) $B;
$ns duplex-link-op $nqueue $h(0) color "green"
$ns duplex-link-op $nqueue $h(0) queuePos 0.25

$ns duplex-link $h([expr $HOPS-1]) $nclient $lineRate [expr $HOP_RTT/2] DropTail;
...
#for {set i 0} {$i<[expr $HOPS-1]} {incr i} {
#
#	$h($i) set multiPath_ 1;
#}

#for {set i 0} {$i<[expr $HOPS-1]} {incr i} {
#	$ns cost $h($i) $h([expr $i+1]) 10;
#	$ns cost $h([expr $i+1]) $h($i) 10;
#	$ns cost $h($i) $nclient [expr ($HOPS-$i)*10];
#	$ns cost $nclient $h($i) [expr ($HOPS-$i)*10];
##	$ns link $h($i) $nclient] cost? ;
#}
#
#
#$ns cost $h([expr $HOPS-1]) $nclient 10;
#$ns cost $nclient $h([expr $HOPS-1]) 10;


#$ns at 0.1 "$ns cost $h([expr $HOPS-1]) $nclient 10";
#$ns at 0.1 "$ns cost $nclient $h([expr $HOPS-1]) 10";

for {set i 0} {$i<$simulationTime} {incr i} {

#	$ns rtmodel-at $i down $h(i) $nclient;
	set remain [expr int(fmod($i, [expr $HOPS-1]))];
	set base [expr int($i*1.0/($HOPS-1))];
	
	set isup [expr int(fmod($base,2))];

	if {$isup==1} {

		if {[expr $remain+2] == $HOPS} {

			$ns rtmodel-at [expr $i+0.0001] up $h(0) $nclient;
#			puts "@$i up h(0)<->client";
		} else {

	
			$ns rtmodel-at [expr $i+0.0001] up $h(0) $h([expr $remain+2]);
#			puts "@$i up h(0)<->h([expr $remain+2])";
		}
	}
	...
	if {$isup==0} {
		...
		if {$remain==0} {
			...
			$ns rtmodel-at [expr $i+0.0001] down $h(0) $nclient;
#			puts "@$i down h(0)<->client";
		} else {
			
			$ns rtmodel-at [expr $i+0.0001] down $h(0) $h([expr $HOPS-$remain]);
#			puts "@$i down h(0)<->h([expr $HOPS-$remain])"
		}
	}
}
...
#	$ns at [expr 0.1+$i/5.0] "$ns cost $h($i) $nclient 999";
#	$ns rtmodel-at [expr 0.2+$i/5.0] cost $h($i) $nclient 999;
#}
